...
| 协议 | 主要优势 | 主要缺点 | 适用场景 |
| --- | --- | --- | --- |
| vmess-httpupgrade | - 更好的流量伪装
- 稳定的长连接
- 良好的穿透性 | - 轻微性能开销
- 部分客户端不支持
- Linux版v2ray不支持 | 需要规避检测的环境，特别是使用最新版V2rayNG的用户 |
| vless-reality | - 轻量高效
- 无需证书
- 更好的TLS伪装
- 双向认证 | - 配置较复杂
- clash-premium不支持
- 需要专业技术知识 | 对安全性和隐私性要求较高的场景，但机器性能不是主要考虑因素 |
| hysteria 2 | - 高速连接
- 支持多路复用
- 资源占用低
- 启动速度快 | - 需要正确配置证书
- 配置复杂
- 部分传统客户端不支持
- UDP可能受限 | 高延迟、高丢包率的网络环境，需要稳定性和低延迟的场景 |
| TUIC | - 快速连接建立
- 优秀的网络性能
- 资源占用低
- 支持0-RTT | - 配置复杂
- sing-box脚本版难用
- UDP可能受限
- 客户端支持有限 | 大文件传输和视频流媒体场景，但目前相比hysteria 2无明显优势 |